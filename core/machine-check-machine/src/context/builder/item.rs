...
impl WContextBuilder {
    pub fn fold_item_struct(&mut self, mut item: ItemStruct) -> Result<WItemStruct, Errors> {
        ...
        let visibility = Self::fold_visibility(item.vis)?;
        ...
        for field in fields_named.named {
            ...
            let visibility = Self::fold_visibility(field.vis)?;
    ...
    pub fn fold_item_impl(&mut self, item: ItemImpl) -> Result<WItemImpl<YBuild>, Errors> {
        ...
        for impl_item in item.items {
            ...
            let err_msg = match impl_item {
                ...
                ImplItem::Fn(impl_item) => {
                    impl_item_fns
                        .push(self.fold_impl_item_fn(impl_item, (item.self_ty.as_ref(), &self_ty)));
                    None
                }
    ...
    pub fn fold_visibility(visibility: Visibility) -> Result<WVisibility, Error> {
        match visibility {
            syn::Visibility::Public(pub_token) => {
                Ok(WVisibility::Public(WSpan::from_syn(&pub_token)))
            }
