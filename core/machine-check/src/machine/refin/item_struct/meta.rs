...
fn proto_first_fn(s: &ItemStruct) -> Result<ImplItemFn, anyhow::Error> {
    ...
    path_rule::apply_to_path(&mut return_type_path, &abstract_type_rules())?;
...
fn proto_increment_fn(s: &ItemStruct) -> Result<ImplItemFn, anyhow::Error> {
    ...
    path_rule::apply_to_path(&mut proto_type_path, &abstract_type_rules())?;
