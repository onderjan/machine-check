...
use super::super::{dual_interval::DualInterval, three_valued::AbstractBitvector};
use crate::{
    abstr::{ArrayFieldBitvector, BitvectorDomain, Boolean, ManipField, PanicResult, Phi, Test},
    bitvector::concrete::{UnsignedInterval, WrappingInterval},
};

#[derive(Clone, Copy, Hash, Default)]
pub struct CombinedBitvector<const W: u32> {
    three_valued: AbstractBitvector<W>,
    dual_interval: DualInterval<W>,
}

impl<const W: u32> CombinedBitvector<W> {
    fn combine(three_valued: AbstractBitvector<W>, dual_interval: DualInterval<W>) -> Self {
        ...
        let interval_bitvec =
            AbstractBitvector::from_interval(near_min.as_bitvector(), far_max.as_bitvector());
    ...
    fn combine_panic_result(
        three_valued: PanicResult<AbstractBitvector<W>>,
...
impl<const W: u32> Phi for CombinedBitvector<W> {
    ...
    fn uninit() -> Self {
        Self {
            three_valued: AbstractBitvector::uninit(),
